...
export default function Stats() {
  ...
  const TitleChip = ({ title }: { title: string }) => (
    ...
    >
      <Typography fontFamily="Coustard" fontSize="1.2rem">
        {title}
      </Typography>
    </Box>
  );
            ...
            <ListItem key={villager} disablePadding>
              <Box display="flex" alignItems="center">
                ...
                <Typography>
                  &nbsp;&nbsp;{photo.trait} days (
                  {dayOrDays(
                    histories.get(villager)!.duration - photo.duration,
                  )}
                  )
                </Typography>
              </Box>
  ...
  return (
    <>
      <Typography>Number of Villagers: {histories.size}</Typography>
      ...
      <Typography>
        Average:{' '}
        {(
          Array.from(histories.values()).reduce((a, b) => a + b.duration, 0) /
          histories.size
        ).toFixed(2)}{' '}
        days
        <br />
      ...
      <Typography>
        ...
        {((photoStats.count / histories.size) * 100).toFixed(2)}%)
        <br />
